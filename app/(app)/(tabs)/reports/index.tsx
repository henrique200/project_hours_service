  ...
  return (
    <View className="flex-1 p-4 bg-white">
      <FlatList
        ...
        ListEmptyComponent={
          <Text className="text-gray-500">Ainda não há relatórios. Gere um na tela de Anotações.</Text>
        }
        renderItem={({ item }) => (
          <View className="border border-gray-200 rounded-xl p-3 gap-2">
            ...
            {item.entries.length ? (
              <View className="gap-1">
                {item.entries.map(ent => (
                  <Text key={ent.date} className="text-gray-700">
            ...
            <View className="flex-row gap-2 mt-2">
              <Pressable onPress={() => deleteReport(item.id)} className="border border-red-500 rounded-xl py-2 px-3">
